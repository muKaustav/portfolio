...
const experiences: Experience[] = [
	{
		...
		highlights: [
			{
				project: 'Reach.r',
				desc: 'Contributed to the Audience management platform with Discover Page (attribute catalog) and Models Page (data mart catalog with category-based filtering) for Marketing/Business teams.',
			},
			{
				project: 'Muxor',
				desc: 'Owned the near real-time audience segmentation engine synchronizing Segment events with user profiles, reducing campaign targeting latency from batch (hours/daily) to near real-time.',
			},
			{
				project: 'QueryQ',
				desc: 'Contributed to the Self-service BigQuery query management platform with scheduling, dependency orchestration, cost controls, and AI Assist for suggesting data quality checks.',
			},
			{
				project: 'Catalogue',
				desc: 'Owned the catalogue service for metadata management and discovery, built on top of OpenMetadata, with MCP server integration for AI agents.',
			},
			{
				project: 'Sentinel',
				desc: 'Owned the data quality service, with Soda Core integration supporting row count, null checks, uniqueness and freshness, among other custom checks.',
			},
			{
				project: 'Projector',
				desc: 'Contributed to the Unified backend platform hosting data services, additionally exposing MCP server for AI agents to use.',
			},
			{
				project: 'Aether',
				desc: 'Owned the multi-agent orchestrator built with Google ADK for Discovery, Modeling, Prediction, Activation, and Operations agents with dynamic MCP tool discovery.',
			},
		],
	},
	{
		...
		<ConfigProvider
			...
			<main
				...
				<div
					...
					<Title
						level={3}
						style={{ margin: 0, color: colors.onSurface, fontWeight: 600 }}>
				...
				<section style={{ marginBottom: 32 }}>
					<Paragraph
						...
						SDE 1 acting as a Platform Engineer, building internal data products
						that democratize data access. I specialize in self-service data
						platforms, real-time infrastructure, and AI-powered tooling for
					...
					<Paragraph
						...
						Currently at Acko's Central Data Team, I own platforms like QueryQ
						(query scheduling & management), Muxor (near real-time audience
						segmentation), and Sentinel (data quality monitoring) for internal
						teams. Previously at Yahoo!, I modernized ETL pipelines and achieved
						70x API latency improvements.
					</Paragraph>
					<Paragraph
						style={{
							color: colors.onSurfaceVariant,
					...
					<ul style={{ margin: 0, paddingLeft: 20 }}>
						{links.map((link) => (
							<li key={link.label} style={{ marginBottom: 8, fontSize: 14 }}>
								<span style={{ color: colors.onSurface, fontWeight: 500 }}>
									{link.label}:
								</span>{' '}
								<Link
									href={link.href}
